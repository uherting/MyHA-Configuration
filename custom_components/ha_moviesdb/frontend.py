...
Registriert `ha-moviesdb-card.js` als Lovelace-"Modul"-Ressource im
Storage-Modus - genau so, wie es der manuelle Weg über "Einstellungen ->
Dashboards -> Ressourcen" auch tut.

Zwei wichtige Details (übernommen aus dem Schwesterprojekt HA SeriesDB, dort
live gefunden):
...
2. Die Ressourcen-Collection wird vom Kern nur "lazy" geladen, ausgelöst
   durch das Frontend (bekannter Home-Assistant-Bug:
   https://github.com/home-assistant/core/issues/165767). Statt passiv auf
   `resources.loaded` zu warten, wird hier deshalb aktiv
...
class LovelaceResourceRegistration:
    ...
    async def async_register(self) -> None:
        ...
        if lovelace is None:
            _LOGGER.debug("HA MoviesDB: Lovelace ist noch nicht geladen.")
            return

        resource_mode = getattr(lovelace, "resource_mode", None)
        if resource_mode != "storage":
            _LOGGER.info(
                "HA MoviesDB: Lovelace läuft im YAML-Modus, die Karten-Ressource "
        ...
        try:
            ...
            _LOGGER.exception(
                "HA MoviesDB: Konnte Lovelace-Ressourcen nicht laden, "
    ...
    async def _async_sync_resource(self, resources: Any) -> None:
        ...
        if not existing:
            _LOGGER.info("HA MoviesDB: Registriere Karte als Lovelace-Ressource.")
        ...
        if current_version != CARD_VERSION:
            _LOGGER.info(
                "HA MoviesDB: Aktualisiere Karten-Ressource auf Version %s.",
                CARD_VERSION,
            )
